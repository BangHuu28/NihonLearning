...
export default function Flashcard() {
    ...
    const levels = [
        { key: "N5", title: "N5 — Sơ cấp 1", vocabCount: "662 từ vựng", lessonCount: "23 bài học" },
        { key: "N4", title: "N4 — Sơ cấp 2", vocabCount: "632 từ vựng", lessonCount: "22 bài học" },
        { key: "N3", title: "N3 — Trung cấp", vocabCount: "1784 từ vựng", lessonCount: "60 bài học" },
        { key: "N2", title: "N2 — Thượng cấp", vocabCount: "1793 từ vựng", lessonCount: "60 bài học" },
        { key: "N1", title: "N1 — Cao cấp", vocabCount: "3463 từ vựng", lessonCount: "116 bài học" }
    ];
    ...
    const renderExamples = (v) => {
        if (v.examples && v.examples.length > 0) {
            return v.examples.map((ex, i) => (
                <div key={i} className="mt-2 text-start p-2 rounded" style={{ fontSize: "14px", background: "var(--jp-bg-surface-alt)", border: "1px solid var(--jp-border)" }}>
            ...
            <div className="col-md-9">
                ...
                {!selectedLevel && (
                    <div className="text-center py-4">
                        <h2 className="fw-bold mb-2" style={{ color: "var(--jp-text-primary)", letterSpacing: "-0.02em" }}>Flashcard</h2>
                            ...
                            {levels.map((lvl) => (
                                <div className="col-md-6 mb-4" key={lvl.key}>
                                    <div
                                        ...
                                        style={{ cursor: "pointer", borderLeft: "3px solid var(--jp-accent)" }}
                                    >
                                        <div className="card-body py-4 d-flex align-items-center justify-content-between">
                                            <div className="text-start">
                                                <h4 className="fw-bold" style={{ color: "var(--jp-text-primary)" }}>{lvl.title}</h4>
                                                <div className="text-muted small">{lvl.vocabCount} | {lvl.lessonCount}</div>
                                            </div>
                                            <span className="badge" style={{ background: "var(--jp-accent)", fontSize: "14px" }}>{lvl.key}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
                ...
                {selectedLevel && !selectedLesson && (
                    <div>
                        <div className="d-flex justify-content-between align-items-center mb-4 pb-2" style={{ borderBottom: "1px solid var(--jp-border)" }}>
                            <h3 className="fw-bold m-0" style={{ color: "var(--jp-text-primary)" }}>Level {selectedLevel}</h3>
                            <button className="btn btn-outline-secondary btn-sm fw-bold" style={{ borderColor: "var(--jp-border)", color: "var(--jp-text-secondary)" }} onClick={() => setSelectedLevel(null)}>
                                ← Chọn Cấp Độ Khác
                            </button>
                        </div>
                                    ...
                                    <div className="col-md-6 mb-3" key={lesson.id}>
                                        ...
                                        >
                                            <div className="card-body d-flex flex-column justify-content-between">
                                                <h5 className="card-title fw-bold" style={{ color: "var(--jp-text-primary)" }}>{lesson.title}</h5>
                ...
                {selectedLesson && (
                    <div>
                        <div className="d-flex justify-content-between align-items-center mb-4 pb-2" style={{ borderBottom: "1px solid var(--jp-border)" }}>
                            <h4 className="fw-bold m-0" style={{ color: "var(--jp-text-primary)" }}>Flashcard: {selectedLesson.title}</h4>
                            <button className="btn btn-outline-secondary btn-sm fw-bold" style={{ borderColor: "var(--jp-border)", color: "var(--jp-text-secondary)" }} onClick={() => setSelectedLesson(null)}>
                        ...
                        {isLoading ? (
                            ...
                            <div className="text-center">
                                ...
                                >
                                    <div className="flashcard-inner">
                                        ...
                                        <div className="flashcard-front">
                                            <span className="badge mb-3" style={{ fontSize: "11px", background: "var(--jp-accent)", letterSpacing: "0.05em" }}>MẶT TRƯỚC</span>
                                            <h1 className="display-3 fw-bold mb-2" style={{ color: "var(--jp-text-primary)", fontFamily: "var(--jp-font-serif)" }}>{vocabularies[currentIdx].word}</h1>
                                            {vocabularies[currentIdx].reading && (
                                                <h4 className="mb-4" style={{ color: "var(--jp-accent)" }}>({vocabularies[currentIdx].reading})</h4>
                                            )}
                                            <button
                                                className="btn btn-link p-0 border-0 bg-transparent hover-scale"
                                                style={{ outline: "none", color: "var(--jp-accent)" }}
                                                onClick={(e) => {
                                            ...
                                            <small className="mt-3 d-block" style={{ color: "var(--jp-text-tertiary)" }}>Nhấn vào thẻ để xem nghĩa tiếng Việt</small>
                                        </div>

                                        {/* Mặt sau: Nghĩa & ví dụ */}
                                        <div className="flashcard-back">
                                            <span className="badge mb-3" style={{ fontSize: "11px", background: "var(--jp-text-secondary)", letterSpacing: "0.05em" }}>MẶT SAU</span>
                                            <h3 className="fw-bold mb-3" style={{ color: "var(--jp-text-primary)" }}>{getMeaningText(vocabularies[currentIdx])}</h3>
                                            <div className="w-100">
                                                {renderExamples(vocabularies[currentIdx])}
                                            </div>
                                            <small className="mt-auto pt-3 d-block" style={{ color: "var(--jp-text-tertiary)" }}>Nhấn vào thẻ để lật lại mặt trước</small>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="d-flex justify-content-center align-items-center gap-4 mt-4">
                                    ...
                                    <span className="fw-bold fs-5" style={{ color: "var(--jp-text-secondary)" }}>{currentIdx + 1} / {vocabularies.length}</span>
                                    <button
